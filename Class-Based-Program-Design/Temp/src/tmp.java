...
abstract class AGameItem implements IGameItem{
  ...
  abstract public IGameItem move(); 
	...
	// produce true if a bullet is outside of the screen; otherwise flase
	public boolean isOffScreen(int width, int height){
...
class ConsLo<T> implements ILo<T> {
  ...
  public <U> ILo<U> buildList(IFuncWA<U> fn, int cont){ throw new RuntimeException("Cannot build a list, already builded.");}
}
...
class MtLo<T> implements ILo<T> {
  ...
  public <U> ILo<U> map(IFunc<T, U> fn) { return new MtLo<U>(); }

  public ILo<T> filter(IPred<T> fn) { return new MtLo<T>(); }

  public <U> U foldr(IFunc2<T, U, U> func, U base) { return base; }

  public ILo<T> append(ILo<T> that){ return that; } 

  public ILo<T> appendEl(T that){ return new ConsLo<T>(that, this); }

  public ILo<T> filterIdx(IPred2<Integer, Integer> fn, int idx, int idxToRemove){ return this; }
...
class UtilityCollision{
  ILo<IGameItem> updatedShips; ILo<IGameItem> updatedBullets;
...
class CollisionWithBullet implements IPred<IGameItem> {
  IGameItem b;
  CollisionWithBullet(IGameItem b){ this.b = b; }
  public boolean apply(IGameItem s){ return this.b.isCollidingWith(s); }
}
...
class SpreadBullets implements IFuncWA<IGameItem>{
  ...
  SpreadBullets(IGameItem b, int counter, int idx){
    this.b = b;
  ...
  public IGameItem apply(){
    ...
    return new Bullet(
          b.getPos(),
          b.getVel().explodeDirection(this.idx, b.getHitCounter() + 2),
          Math.min(maxSize, b.getRadius() + incrSize),
          b.getHitCounter() + 1);
  }
}
...
class GenerateBulletWithRef implements IFuncWA<IGameItem>{
  IGameItem ref;
...
class Collision implements IFunc2<IGameItem, UtilityCollision, UtilityCollision>{
  ...
  public UtilityCollision apply(IGameItem b, UtilityCollision u){   
    ...
    if(updatedShips.length() != los.length()){
      ...
      ILo<IGameItem> spreadBullets = new MtLo<IGameItem>().buildList(
        new GenerateBulletWithRef(
          new SpreadBullets(b, b.getHitCounter() + 2, 0).apply()
        ), 1);
      return new UtilityCollision().setUpdatedShips(updatedShips).addBullets(spreadBullets);
    }else{
      return new UtilityCollision().addBullet(b);
    }
  }
}
		...
		if(new Random().nextInt(2) == 0){
			return new Ship(
				new MyPosn(
					...
					Math.max(edge, new Random().nextInt(this.height) - edge)), 
...
class GenerateBullet implements IFuncWA<IGameItem>{
  ...
  public IGameItem apply()
  {
      ...
      return new Bullet(new MyPosn(this.width/ 2, this.height - edge), new MyPosn(0, -3), RADIUS_BULLET, 0);
  }
}
...
class OffScreen implements IPred<IGameItem> {
  ...
  public boolean apply(IGameItem t) {
		return t.getPosX() <= WIDTH && t.getPosY() <= HEIGHT && t.getPosX() >= 0 && t.getPosY() >= 0;
  }
}
...
class RemoveByIndex implements IPred2<Integer, Integer>{
  public boolean apply(Integer idx, Integer idxToRemove){ return idx != idxToRemove; }
}
...
class MyGame extends World {
	...
	MyGame(int bullets){
		...
		this.currentShips = new MtLo<IGameItem>().buildList(new GenerateShip(this.WIDTH, this.HEIGHT), new Random().nextInt(4));
		this.bullets = bullets;
		// The current bullets on the screen
		this.currentBullets = new MtLo<IGameItem>();
	}

	MyGame(int bullets, int destroyedShip, ILo<IGameItem> currentShips, int currentTick, ILo<IGameItem> currentBullets){
		this.WIDTH  =  WIDTH;
		this.HEIGHT = HEIGHT;
		this.bullets = bullets;
    ...
    UtilityCollision updatedElements = this.currentBullets.foldr(new Collision(this.currentShips), new UtilityCollision());
    ...
    ILo<IGameItem> newShips = 
      new MtLo<IGameItem>().buildList(new GenerateShip(this.WIDTH, this.HEIGHT), new Random().nextInt(4));
		return new MyGame(this.bullets, this.destroyedShip, 
			this.currentShips.append(newShips),
			this.currentTick,
			this.currentBullets);
		}else{
			return this;
    ...
    // Show the current number of destroyed Ships on the scene
		s = s.placeImageXY(new TextImage("Ship: " + this.destroyedShip, Color.black), this.WIDTH - 50, 30);
	...
	public MyGame onKeyEvent(String key) {
       ...
       ILo<IGameItem> newBullets = new MtLo<IGameItem>().buildList(new GenerateBullet(this.WIDTH, this.HEIGHT), 1);
			 return new MyGame(
        this.bullets - 1,
        this.destroyedShip,
        this.currentShips,
...
class Examples {
  ...
  // List of ships
  ILo<IGameItem> los0 = new ConsLo<IGameItem>(s0, new ConsLo<IGameItem>(s1, new ConsLo<IGameItem>(s2, mtIGameItem)));

  ILo<IGameItem> los1 = new ConsLo<IGameItem>(s3, new ConsLo<IGameItem>(s4, new ConsLo<IGameItem>(s5, mtIGameItem)));

  ILo<IGameItem> los2 = new ConsLo<IGameItem>(s3, new ConsLo<IGameItem>(s4, mtIGameItem));

  ILo<IGameItem> los3 = new ConsLo<IGameItem>(s0, new ConsLo<IGameItem>(s1, new ConsLo<IGameItem>(s2,
      new ConsLo<IGameItem>(s3, new ConsLo<IGameItem>(s4, new ConsLo<IGameItem>(s5, mtIGameItem))))));

  ILo<IGameItem> los4 = new ConsLo<IGameItem>(s3, new ConsLo<IGameItem>(s4,
      new ConsLo<IGameItem>(s5, new ConsLo<IGameItem>(s3, new ConsLo<IGameItem>(s4, mtIGameItem)))));
  ...
  IFuncWA <IGameItem> gBWR = new GenerateBulletWithRef(new Bullet(p0, v0, RADIUS_BULLET, 0));
  ...
  boolean testMoveAll(Tester t) {
    IGameItem b0_ = new Bullet(new MyPosn(WIDTH_GAME / 2, HEIGHT_GAME - 10), new MyPosn(0, -6), RADIUS_BULLET, 0);
    ILo<IGameItem> lob4 = new ConsLo<IGameItem>(b0,
      new ConsLo<IGameItem>(b0, new ConsLo<IGameItem>(b0, new ConsLo<IGameItem>(b0, mtIGameItem))));
    return 
    t.checkExpect(los0.map(mvA), los1)
    &&
    t.checkExpect(lob0.map(mvA), lob1)
    &&
    t.checkExpect(
      new MtLo<IGameItem>().buildList(gB, 4)
      .map(mvA), lob4)
    ;
  }
  ...
  return
  t.checkExpect(los0.filter(oS), los0_)
  &&
  t.checkExpect(los1.filter(oS), los2)
  ;
  }
  ...
  return
    t.checkExpect(los0.length(), 3)
    &&
    t.checkExpect(los2.length(), 2)
    &&
    t.checkExpect(lob0.length(), 3)
    &&
    t.checkExpect(lob2.length(), 2)
    &&
  ...
  //   &&
  //   t.checkExpect(new ConsLoBullet(b5, mtBullet).collision(los4), new
  //   IndexUtility(2,0))
  //   &&
  //   t.checkExpect(new ConsLoBullet(b5, mtBullet).collision(los1), new
  //   IndexUtility(2,0))
  //   &&
  //   t.checkExpect(new ConsLoBullet(new Bullet(p6, v3, RADIUS_BULLET, 0),
  //   mtBullet).collision(los1), new IndexUtility(0, 0))
  //   ;
  // }
  ...
  ILo<IGameItem> los3_ = new ConsLo<IGameItem>(s0, new ConsLo<IGameItem>(s1, new ConsLo<IGameItem>(s3, new
  ConsLo<IGameItem>(s4, new ConsLo<IGameItem>(s5, mtIGameItem)))));
  return
    t.checkExpect(los0.filterIdx(rBI, 0, 0), los0_)
    &&
  ...
  // return
  // t.checkConstructorException(new IllegalArgumentException("The player cannot
  // start without less or equal 0 bullets."), "MyGame", -1)
  // &&
  // t.checkConstructorException(new IllegalArgumentException("The player cannot
  // start without less or equal 0 bullets."), "MyGame", 0)
  // ;
  // }
  //
  ...
  ILo<IGameItem> lob0_ = new ConsLo<IGameItem>(b0, new ConsLo<IGameItem>(b0, new ConsLo<IGameItem>(b0, mtIGameItem)));
    return
    t.checkExpect(new MtLo<IGameItem>().buildList(gBWR, 3), lob0_)
    // &&
    // t.checkExpect(new MtLo<IGameItem>().buildList(new GenerateShip(this.WIDTH_GAME, this.HEIGHT_GAME), new Random().nextInt(4)), lob0_)
    ;
  }
  ...
  // return
  // // MyGame(int bullets, int destroyedShip, ILoShip currentShips, int
  // currentTick, ILoBullet currentBullets){
