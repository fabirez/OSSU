...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname fold-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define (fold-element fn-el fn-loe b e)
  (local [(define (fn-for-element e)
            (fn-el (elt-name e)    ;String
                   (elt-data e)    ;Integer
                   (fn-for-loe (elt-subs e))))
          ...
          (define (fn-for-loe loe)
            (cond [(empty? loe) b]
                  [else
                   (fn-loe (fn-for-element (first loe))
                           (fn-for-loe (rest loe)))]))]
    (fn-for-element e)))
...
(define (sum-data e) 
  (local [(define (add-el n d r) (+ d r))]
    (fold-element add-el + 0 e)))
...
(define (all-names e)
  (local [(define (get-name n d r) (cons n r))]
    (fold-element get-name append empty e))) 
...
;; Because of the big and large *recursion* since you need to search in the whole tree 
